...
- Each node controls the values on its own ports. This allows updates to take place deterministically and in parallel.
- Nodes that consume enrgy take whatever positive amount of energy is available and notify the source agent of the amount consumed. Since this could be taking place many times in parallel, the source agent may have a negative energy for some time before it recovers.
- Nodes can be for literally anything. It should be easy to abstract away the entire MAN into a node of its own.
- It should be easy to use AInimal zookeeper to rearrange the MAN connectivity. This allows Limboids and Computatra to perform runtime model introspection as part of self-directed ML.
- You can train only partial subsets of the MAN in various specific training paradigms in a pythonic way. I.E.: developers have the ability to extract a `q_function` from `man` and then train `q_function` in a standard RL trianing loop. They should be able to extract multiple overlapping subnetworks like `q_function`, `sac_policy`, `trpo_policy`, `ar_model`, `clutering_model`, etc. and simultaneously train them all at once (i.e.: gradients are computed and applied in parallel or at least interleaved in close serial succession).
